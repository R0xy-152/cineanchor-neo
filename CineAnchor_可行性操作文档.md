...
文档版本：V1\.2
更新日期：2026\-06\-23
更新说明：根据当前 Spike 进度保守更新；删除 Anchor（UE5 沙盒游戏）建议；记录 Route 0/1/2/4 已通过、Route 3 技术链路可跑但因文字腐蚀未通过验收。
...
## **当前 Spike 进度（截至 2026\-06\-23）**

|**路线**|**当前结论**|**关键说明**|
|---|---|---|
|**Route 0：GLB 渲染闭环**|通过|Blender Headless \+ FFmpeg 链路跑通；Eevee 已作为默认渲染引擎，Cycles 保留为后期画质选项。|
|**Route 1：PNG 2\.5D 视频**|通过|透明 PNG 可生成更接近宣传视频的 2\.5D 片段，透明通道和主体入框验证通过。|
|**Route 2：JSON 驱动模板渲染**|通过|Project JSON 可驱动模板、素材路径、镜头运动、时长、fps、分辨率、标题和副标题。|
|**Route 3：ComfyUI AI 美化**|未通过验收|ComfyUI API 与 keyframe 处理链路已跑通，但直接对带文字的最终帧做整帧 img2img 会腐蚀字幕；暂不进入 Web 渲染链。|
|**Route 4：Web 到本地渲染串联**|通过|本地 Web UI 可调用 FastAPI，触发 Blender JSON 渲染、FFmpeg 合成、轮询状态并下载 MP4。|
...
- ComfyUI API 可达，Python 脚本可通过 API 提交工作流并下载增强图。
- 4 张 character\_intro keyframe 和 4 张 product\_orbit keyframe 已完成增强并生成 side\-by\-side 对比图。
- RealVisXL\_V5\.0\_Lightning\_fp16\.safetensors 工作流可运行，单帧耗时约 3\.2\-7\.4 秒。
- 直接对最终渲染帧做整帧 img2img 会破坏 character\_intro 的 subtitle，4/4 张增强图出现文字腐蚀。
- 当前策略不通过 Route 3 验收。下一次尝试应改为「先增强无文字画面，再叠加文字」，或对文字区域做 mask/原图恢复。
...
|**前提说明 **以下架构基于当前 Spike 结论保守调整。Route 0/1/2/4 已通过；Route 3 技术链路可跑但未通过文字完整性验收，因此 ComfyUI 暂不进入 Web 渲染链，后续仅作为可选增强方向继续验证。|
|---|
...
|**3D 渲染**|Blender 5\.1\.2 Headless（已验证），Eevee 引擎（MVP 默认），Cycles 保留为后期画质选项|
|**AI 美化**|ComfyUI 本地部署，SDXL 模型，通过 REST API 调用；当前仅作独立 Spike，不接入 Web 渲染链|
...
|---|---|
|**时间**|2 周|
|**目标**|验证路线 0/1/2/4 已通过；Route 3 已证明可调用但未通过文字完整性验收|
...
|---|---|
|**时间**|4\-6 周|
|**目标**|新增 product\_orbit、prop\_showcase 模板；继续验证安全的 AI 美化路径|
|**P0 功能**|美化对比（原始 vs 美化）可在前端展示；AI 美化必须保护标题/字幕文字不被改写|
|**Kill 判据**|AI 美化后的视频，相比原始版本，非开发用户有明显正向反馈（「这个能发」），且标题/字幕无明显腐蚀|
...
|**显存竞争**|Blender \+ ComfyUI 同时运行可能导致 OOM；当前测试中 ComfyUI 处理后会保留较高显存占用|串行执行：先完成 Blender 渲染，再启动 ComfyUI；必要时提供卸载模型或重启 ComfyUI 的操作|
...
|**ComfyUI 版本依赖**|ComfyUI 工作流与模型版本强绑定，升级后工作流可能失效；当前已测 ComfyUI 0\.25\.0 与 RealVisXL\_V5\.0\_Lightning\_fp16|锁定 ComfyUI 版本；工作流 JSON 入 git；提供版本检查脚本|
|**AI 美化破坏文字**|对最终帧做整帧 img2img 会改写或腐蚀标题/字幕|优先增强无文字画面，再后期叠加文字；或对文字区域做 mask/原图恢复|
...
|**已完成**|验证路线 0（3D 基础渲染闭环），Eevee 作为 MVP 默认渲染引擎|
|**已完成**|验证路线 1（PNG 2\.5D 视频），透明 PNG 宣传片方向通过|
|**已完成**|验证路线 2（JSON 模板化渲染），character\_intro 与 product\_orbit 跑通|
|**已完成**|验证路线 4（Web → FastAPI → Blender 串联），本地 Web 生成 MP4 跑通|
|**已完成但未通过验收**|验证路线 3（ComfyUI AI 美化），API 与 keyframe 链路跑通，但最终帧文字被腐蚀|
|**下一步**|决定是否重试 Route 3：改为增强无文字帧后再叠加文字，或对文字区域做 mask/恢复|
|**Spike 收尾**|根据验证结论整理可保留代码、文档、环境依赖和下一阶段 V0\.1 范围|
