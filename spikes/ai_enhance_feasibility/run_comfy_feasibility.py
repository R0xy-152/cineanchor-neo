...
DEFAULT_WORKFLOW = SCRIPT_DIR / "workflows" / "sdxl_img2img_api.json"
DEFAULT_POSITIVE = (
    "polished mobile game promotional keyframe, cinematic lighting, richer color, "
    "clean highlights, crisp edges, premium game ad still, preserve exact subject "
    "silhouette, preserve exact pose, preserve exact composition, no text"
)
DEFAULT_NEGATIVE = (
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="ComfyUI feasibility denoise sweep")
...
def run() -> int:
    ...
    try:
        ...
        client_id = f"cineanchor-ai-gate-{uuid.uuid4().hex}"
        ...
        for denoise in args.denoise:
            ...
            for index, raw_frame in enumerate(raw_frames, start=1):
                ...
                output_prefix = f"cineanchor_ai_gate_{args.case}_{args.experiment}_{denoise_label(denoise)}_{raw_frame.stem}_{uuid.uuid4().hex[:8]}"
                ...
                print(f"AI_GATE_COMFY_FRAME={enhanced_path}")
                print(f"AI_GATE_COMFY_DENOISE={denoise:.2f}")
                print(f"AI_GATE_COMFY_SECONDS={elapsed:.3f}")

        report["status"] = "PASS"
        report["gpu_after"] = gpu_snapshot()
        report["finished_at_epoch"] = time.time()
        write_json(report_path, report)
        print(f"AI_GATE_COMFY_REPORT={report_path}")
        return 0
        ...
        report["finished_at_epoch"] = time.time()
        write_json(report_path, report)
        print(f"AI_GATE_COMFY_ERROR={exc}", file=sys.stderr)
        print(f"AI_GATE_COMFY_REPORT={report_path}", file=sys.stderr)
        return 1
