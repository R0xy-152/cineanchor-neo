...
DEFAULT_WORKFLOW = SCRIPT_DIR / "workflows" / "conservative_sdxl_img2img_api.json"
DEFAULT_POSITIVE = (
    "conservative game promotional keyframe enhancement, preserve exact character, "
    "preserve exact product shape, preserve all text, clean lighting, crisp edges, "
    "subtle contrast, no redesign"
)
DEFAULT_NEGATIVE = (
    "deformed subject, changed face, changed costume, changed product geometry, "
    "warped logo, corrupted text, extra text, flicker, blur, artifacts, oversaturated"
)
...
def run() -> int:
    ...
    report_path = output_root / "comfy_run_report.json"
    ...
    try:
        ...
        if len(raw_frames) < 8:
            raise ComfyError(f"Expected at least 8 raw keyframes, found {len(raw_frames)} under {output_root}")

        client_id = f"cineanchor-route3-{uuid.uuid4().hex}"
        random.seed(3073)
        for index, (case, raw_frame) in enumerate(raw_frames, start=1):
            enhanced_dir = output_root / case / "keyframes_enhanced"
            enhanced_path = enhanced_dir / raw_frame.name
            output_prefix = f"cineanchor_route3_{case}_{raw_frame.stem}_{uuid.uuid4().hex[:8]}"
            ...
            print(f"ROUTE3_COMFY_FRAME={enhanced_path}")
            print(f"ROUTE3_COMFY_SECONDS={elapsed:.3f}")

        report["status"] = "PASS"
        report["gpu_after"] = gpu_snapshot()
        report["finished_at_epoch"] = time.time()
        write_json(report_path, report)
        print(f"ROUTE3_COMFY_REPORT={report_path}")
        return 0
        ...
        report["finished_at_epoch"] = time.time()
        write_json(report_path, report)
        print(f"ROUTE3_COMFY_ERROR={exc}", file=sys.stderr)
        print(f"ROUTE3_COMFY_REPORT={report_path}", file=sys.stderr)
        return 1
